...
export async function mealDelete(id: string, date: number) {
    try {
        ...
        const dataByDate = storageData.find((item) => item.title === dateFormat(date, 'date'));

        if (dataByDate) {
            const filteredMeals = dataByDate.data.filter((meal) => meal.id !== id);
            dataByDate.data = filteredMeals;
            const updatedStorageDataWhenNoMeals = storageData.filter((item) => item.title !== dataByDate.title);
